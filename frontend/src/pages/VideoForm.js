...
const VideoForm = () => {
  ...
  const loadVideo = async () => {
    try {
      ...
      setFormData({
        ...
        etapaFormativaId: video.etapaFormativaId || '',
        localId: video.localId || '',
        pessoaId: video.pessoaId || ''
      });
    } catch (error) {
      toast.error('Erro ao carregar video');
      navigate('/videos');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.url && !arquivo && !isEditing) {
      toast.error('Informe uma URL ou selecione um arquivo de video');
      return;
    }
    ...
    try {
      ...
      toast.error(error.response?.data?.error || 'Erro ao salvar video');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        ...
        <h2>{isEditing ? 'Editar Video' : 'Novo Video'}</h2>
      </div>
      ...
      <form className="form-card" onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <div className="file-input-wrapper">
            <div className="file-input-label">
              <FiUpload size={24} />
              <span>{arquivo ? arquivo.name : 'Clique para selecionar um video'}</span>
            </div>
            <input
              type="file"
              onChange={handleFileChange}
              accept=".mp4,.avi,.mov,.wmv,.mkv,.webm"
            />
          </div>
          {arquivo && (
        ...
        <div className="form-actions">
          <button type="button" className="btn-secondary" onClick={() => navigate('/videos')}>
            Cancelar
          </button>
